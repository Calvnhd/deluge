...
def sync_files(sd_path: Path, repo_path: Path, trash_path: Path) -> SyncStats:
    """
    ...
    # Collect all relevant files from SD card
    sd_files: set[str] = set()
    for ext in ('*.xml', '*.XML', '*.wav', '*.WAV'):
        for f in sd_path.rglob(ext):
            rel = f.relative_to(sd_path)
            sd_files.add(str(rel))
    
    # Collect all relevant files from repo
    ...
    for ext in ('*.xml', '*.XML', '*.wav', '*.WAV'):
        for f in repo_path.rglob(ext):
            ...
            repo_files.add(str(rel))
    
    # Process files from SD card (add/modify)
    for rel_path in sorted(sd_files):
    ...
    deleted_files = repo_files - sd_files
    if deleted_files:
    ...
    return SyncStats(added, modified, trashed, unchanged)
...
def main() -> int:
    ...
    # Perform sync
    try:
        stats = sync_files(sd_path, repo_deluge_path, trash_path)
        print_summary(stats)
